 ...
 *	
 *	Recurrence Equation:
 *		s(1..j) contains the word to change from
 *		t(1..j) contains the word to change to
 *		
 *		M(i, j) := M(i-1, j-1) + copyCost,  if s(i) == t(j)
 *		
 *		M(i, j) := min( M(i-1, j  )	+ insertionCost,
 *						M(i,   j-1)	+ deletionCost,
 *						M(i-1, j-1)	+ substitutionCost)
 *						
 *		M(i, 0) := i * insertionCost
 *		M(0, j) := j * deletionCost
 *	
 *	References: I had watched this video: https://youtu.be/ocZMDMZwhCY?t=1873 
 *	previously (studying for exam), so I guess I kind of had help working the 
 *	project.  A very general method of solving the problem and it's
 *	recussive definition are discussed.
 */
#include <iostream>
#include <string>
#include <vector>
